...
class MultiNN(object):
    ...
    def add_layer(self, num_nodes, activation_function):
        ...
        self.biases.append(tf.random.normal([num_nodes]))
    ...
    def predict(self, X):
        ...
        """
        X_temp= X
        for a in range(0,len(self.activations)):
            net=tf.add(tf.matmul(X_temp,self.weights[a]),self.biases[a])
            op=self.activations[a](net)
            X_temp=op
        return op
    ...
    def train(self, X_train, y_train, batch_size, num_epochs, alpha=0.8, regularization_coeff=1e-6):
        ...
        batch_data=batch_data.batch(batch_size)
        for _ in range(num_epochs):
            for x,y in batch_data:
                with tf.GradientTape(persistent=True) as tape:
                    predict_x=self.predict(x)
                    loss=self.cross_entropy_loss(y,predict_x)
                    for w in range(len(self.weights)):
                        loss_w,loss_b=tape.gradient(loss,[self.weights[w],self.biases[w]])
                        self.weights[w].assign_sub(alpha*loss_w)
                        self.biases[w].assign_sub(alpha*loss_b)
    ...
    def calculate_percent_error(self, X, y):
        ...
        """
        return np.average(np.where((y-np.argmax(self.predict(X),axis=1))!=0,1,0))
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        """
        confusion_mtrx = np.zeros((self.input_dimension, self.input_dimension))
        #predict_x=self.predict(X)
        #p_temp=np.transpose(predict_x)
        #print(p_temp)
        z = np.argmax(self.predict(X), axis=1)
        for i in range(0, len(y)):
            confusion_mtrx[y[i]][z[i]]+=1
